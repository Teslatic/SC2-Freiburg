...
from assets.RL.AtariNet import DQN, ExtendedDQN, ExtendedDQNXY
...
class DQN_module():
    ...
    def _build_model(self):
        ...
        """
        net = ExtendedDQNXY(self.history_length, self.dim_actions).to(self.device)
        target_net = ExtendedDQNXY(self.history_length, self.dim_actions).to(self.device)
    ...
    def calculate_q_values(self):
        ...
        """
        # forward pass
        self.state_q_values, self.x, self.y = self.net(self.state_batch)
        ...
        self.next_state_q_values, self.next_state_Qx, self.next_state_Qy = self.target_net(self.next_state_batch)
